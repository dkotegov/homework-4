...
class Discussion(BaseElement):
    ...
    def get_last_edited_comment(self):
        elms = self.driver.find_elements(By.XPATH, '//*[@class="d_comment_w d_comment_w__avatar __me show-on-hover __edited"]')
        if elms is None:
            return None
        else:
            if len(elms) == 0:
    ...
    def get_last_comment_change_button(self):
        ...
        if elms is None:
            ...
            if len(elms) == 0:
                return None
            self.element = elms[-1].find_element(By.CSS_SELECTOR, 'div.d_comment_w_center > div.d_comment_right_w > div.d_comment_text_w > div.d_comment_r > div.d_comment_act_w > a.d_comment_act.d_comment_act_del.d_comment_act_spam.ic10.ic10_edit.fade-on-hover')
            return self