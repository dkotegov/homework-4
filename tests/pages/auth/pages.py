...
LOGIN_LOGIN_ID = 'js-email-login'
LOGIN_PASSWORD_ID = 'js-password-login'
LOGIN_LOGIN_ERROR = 'error-js-email-login'
LOGIN_PASSWORD_ERROR = 'error-js-password-login'
...
class Pages(BasePages):
    ...
    @staticmethod
    def enter_login_login(login):
        a.wait_for_load(LOGIN_LOGIN_ID)
        element = a.find_element_by_id(LOGIN_LOGIN_ID)
        element.wait_and_click()
        element.send_keys(login)

    @staticmethod
    def enter_login_password(password):
        a.wait_for_load(LOGIN_PASSWORD_ID)
        element = a.find_element_by_id(LOGIN_PASSWORD_ID)
        element.wait_and_click()
        element.send_keys(password)

    @staticmethod
    def find_login_login_error():
        a.wait_for_load(id_locator=LOGIN_LOGIN_ERROR)

    @staticmethod
    def find_login_password_error():
        a.wait_for_load(id_locator=LOGIN_PASSWORD_ERROR)
