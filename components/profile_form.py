...
class ProfileForm(Component):
    ...
    OLD_PASSWORD_ERROR = '//input[@id="inputOldPasswordError"]'
    NEW_PASSWORD = '//input[@id="inputPassword"]'
    NEW_PASSWORD_REPEAT = '//input[@id="inputPasswordRepeat"]'
    NEW_PASSWORD_ERROR = '//input[@id="inputPasswordError"]'
    ...
    def is_invalid_new_password(self):
        try:
            return WebDriverWait(self.driver, 1).until(lambda d: len(d.find_element_by_xpath(self.NEW_PASSWORD_ERROR).text) != 0)
        except TimeoutException:
            return False

    def is_invalid_email(self):
        try:
            return WebDriverWait(self.driver, 1).until(lambda d: len(d.find_element_by_xpath(self.EMAIL_ERROR).text) != 0)
        except TimeoutException:
            return False
